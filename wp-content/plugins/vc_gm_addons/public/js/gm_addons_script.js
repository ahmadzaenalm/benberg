...
;(function ($, window, document, undefined) {
    ...
    if ($('.gm-map__wrapper').length) {
        $('.gm-map__wrapper > div').each(function () {
                initializeMap(this);
        });
    }
    ...
    function initializeMap(_this) {
        ...
        // default coords
        var maplat = 49.838383;
        var maplng = 24.0319108;
        ...
        if (Boolean(optionsMap.is_reload_on_resize) === true) {
            // console.log(Boolean( optionsMap.is_reload_on_resize ));
                        ...
                        'address': String(item.marker_address)
                            ...
                            if (status == google.maps.GeocoderStatus.OK) {
                                if (status != google.maps.GeocoderStatus.ZERO_RESULTS) {
                                    ...
                                    if (optionsMap.clustering === 'yes') {
                                        var markerCluster = new MarkerClusterer(map, marker,
                                            {imagePath: window.location.origin + 'wp-content/plugins/vc_gm_addons/public/images/default_marker'});
                                    }
                                    ...
                                    marker.addListener('click', function () {
                                        if (item.icon_animation !== 'none') {
                                            if (marker.getAnimation() !== null) {
                                                ...
                                                // marker.setAnimation(google.maps.Animation[item.icon_animation]);
                                            } else {
                                                marker.setAnimation(google.maps.Animation[item.icon_animation]);
                                            }
                                        }
